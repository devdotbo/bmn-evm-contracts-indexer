...
ponder.on(
  "CrossChainEscrowFactoryV2:SrcEscrowCreated",
  async ({ event, context }) => {
...
ponder.on(
  "CrossChainEscrowFactoryV2:DstEscrowCreated",
  async ({ event, context }) => {
    const chainId = context.chain.id;
...
ponder.on(
  "CrossChainEscrowFactoryV2:ResolverWhitelisted",
  async ({ event, context }) => {
    const chainId = context.chain.id;
...
ponder.on(
  "CrossChainEscrowFactoryV2:ResolverAdded",
  async ({ event, context }) => {
    const chainId = context.chain.id;
...
ponder.on(
  "CrossChainEscrowFactoryV2:ResolverRemoved",
  async ({ event, context }) => {
    const chainId = context.chain.id;
    const { resolver } = event.args;
    const id = `${chainId}-${resolver}`;
...
ponder.on(
  "CrossChainEscrowFactoryV2:ResolverSuspended",
  async ({ event, context }) => {
    const chainId = context.chain.id;
...
ponder.on(
  "CrossChainEscrowFactoryV2:ResolverReactivated",
  async ({ event, context }) => {
    const chainId = context.chain.id;
    const { resolver } = event.args;
    const id = `${chainId}-${resolver}`;
...
ponder.on(
  "CrossChainEscrowFactoryV2:AdminAdded",
  async ({ event, context }) => {
    const chainId = context.chain.id;
...
ponder.on(
  "CrossChainEscrowFactoryV2:AdminRemoved",
  async ({ event, context }) => {
    const chainId = context.chain.id;
    const { admin } = event.args;
    const id = `${chainId}-${admin}`;
...
ponder.on(
  "CrossChainEscrowFactoryV2:EmergencyPause",
  async ({ event, context }) => {
    const chainId = context.chain.id;
...
ponder.on(
  "CrossChainEscrowFactoryV2:SwapInitiated",
  async ({ event, context }) => {
    const chainId = context.chain.id;
    const { escrowSrc, maker, resolver, volume, srcChainId, dstChainId } =
      event.args;
    ...
    const resolverId = `${chainId}-${resolver}`;
    const resolverRecord = await context.db.find(resolverWhitelist, {
      id: resolverId,
    });
    if (resolverRecord) {
      await context.db.update(resolverWhitelist, {
...
ponder.on(
  "CrossChainEscrowFactoryV2:SwapCompleted",
  async ({ event, context }) => {
    const chainId = context.chain.id;
...
ponder.on(
  "CrossChainEscrowFactoryV2:InteractionExecuted",
  async ({ event, context }) => {
    const chainId = context.chain.id;
    const { orderMaker, interactionTarget, interactionHash, timestamp } =
      event.args;
...
ponder.on(
  "CrossChainEscrowFactoryV2:InteractionFailed",
  async ({ event, context }) => {
    const chainId = context.chain.id;
...
ponder.on(
  "CrossChainEscrowFactoryV2:MetricsUpdated",
  async ({ event, context }) => {
    const chainId = context.chain.id;
